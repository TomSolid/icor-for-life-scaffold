# AGENTS.md - ICOR for Life Scaffold

This is the canonical, runtime-independent entry contract. Read it first,
every session. `CLAUDE.md` imports it; root `AGENT.md` points here for
compatibility. Per-specialist `06 AI Team/Agents/<Name>/AGENT.md` files
remain individual role contracts, not copies of this root contract.
...
specialist's contract and task context. `.claude/agents/` supplies Claude
Code adapters; it is not a universal dispatch API. A dispatched specialist
