...
(`03 WiP/2026-09-14-scaffold-skills-hooks-audit/report.md` section 6.1), under
...
WHAT IT BUILDS, AND FROM WHAT
    ...
    skills        SOP / Workstream frontmatter (skill_name, skill_summary,
                  ...
                  -> `06 AI Team/AI Team Knowledge/Skills/<name>/SKILL.md`
                     the canonical, plain, portable file
                  -> `.claude/skills/<name>/SKILL.md`  adapter, Claude-only
                     frontmatter and the injected prerun live ONLY here
                  -> `.agents/skills/<name>`           link to the canonical
                     folder, read by Codex, Gemini and Cursor
    ...
    pointers      -> `GEMINI.md`, `.codex/config.toml`, written only when
                     absent. `CLAUDE.md` is never written or overwritten.
...
def find_root(start=None):
    """Walk up until a folder holds both AGENTS.md and `06 AI Team/`.
    ...
    for p in [here] + list(here.parents):
        if (p / "AGENTS.md").is_file() and (p / "06 AI Team").is_dir():
            return p
...
SCRIPT_RE = re.compile(
    r"(?:06 AI Team/AI Team Knowledge/)?Scripts/[A-Za-z0-9_./-]+\.(?:py|sh|mjs|js)")
...
def skill_body(sk, root, prerun_line=None, dispatchable=()):
    who = sk["owner"] or "the owning specialist"
    lines = []
    lines.append("")
    slug = (sk["owner"] or "").lower()
    if slug and slug in dispatchable:
        lines.append("You are %s. If you are the orchestrator, dispatch subagent "
                     "`%s` with this brief; if you are already %s, act directly."
                     % (who, slug, who))
    else:
...
def render_canonical_skill(sk, root, dispatchable=()):
    """The portable file: name, description, body, and nothing host-specific.

    Plain open-spec shape, which is the part Pax found genuinely universal
    across Claude Code, Codex, Gemini CLI, Cursor, Copilot and OpenCode. Every
    Claude-only field lives in the .claude adapter instead, so this file
    degrades to "works" rather than to "silently skipped" on the other five.
    """
    fm = ["---", "name: %s" % sk["name"],
          "description: %s" % json.dumps(description_for(sk)), "---"]
    return _with_md_header(fm + skill_body(sk, root, None, dispatchable),
...
def render_claude_skill(sk, root, dispatchable=()):
    """The Claude Code adapter. Every field Pax marked Claude-only lives HERE.
    ...
    if sk["prerun"]:
        ...
        fm.append("user-invocable: true")
    fm.append("---")
    lines = fm + skill_body(sk, root, prerun_line, dispatchable)
...
# working directory. A session started in `03 WiP/` therefore pointed every hook
# at `03 WiP/.claude/hooks/<guard>.py`, python could not open it, and python
...
def build(root):
    ...
    pointer("GEMINI.md", lambda: _with_md_header(
        ["@AGENTS.md", "",
         "Gemini CLI reads this file. `AGENTS.md` next to it is the contract; "
         "there is no second copy of it here.", ""],
        "AGENTS.md", insert_at=0))
