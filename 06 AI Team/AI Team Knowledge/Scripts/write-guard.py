...
Two rules, and only two:
  ...
  1. PROTECTED PATHS. The user's raw daily notes under
     ...
     contract `06 AI Team/Agents/<Name>/AGENT.md`, plus that contract's
...
AGENT_MARKER = re.compile(r"^06 AI Team/Agents/([^/]+)/\.hiring$")
...
# evening, `03 WiP/2026-09-14-scaffold-skills-hooks-audit/vex-security-gate.md`
...
def command_paths(command, cwd):
    """(absolute paths this command would WRITE, unlocked_by_prefix, unresolvable).
        ...
        if head in _INTERPRETERS:
            ...
            if how is not None:
                ...
                if sub is not None:
                    ...
                    state["unresolvable"].extend(sub[2])
                else:
        ...
        if headers:
            ...
            try:
                ...
                if unresolvable:
                    ...
                    for _who, _how, seg_cwd, program in unresolvable:
...
def relative(path, payload):
    ...
    if not path:
        return None
    try:
        # A relative path in a patch header or a shell command is relative to
        # the SESSION's working directory, which the payload carries. Resolving
        # it against this process's own cwd answered a different question
        # whenever the hook was not started in the session folder.
        if not os.path.isabs(path) and payload.get("cwd"):
            path = os.path.join(str(payload.get("cwd")), path)
...
def main():
    ...
    if shell_unlocked and not unlocked:
        unlocked = True
        print("write-guard: %s=1 prefix on this command, guard stood down for this "
              "one call." % UNLOCK_ENV, file=sys.stderr)

    path = paths[0] if paths else ""
    rel = relative(path, payload)
    rels = [relative(c, payload) for c in paths]
    if not unlocked:
        root = payload_root(payload)
        for crel in rels:
            if not crel:
                continue
            why = protected_reason(crel)
            if why and hiring_open(root, crel):
    ...
    scan = True
    if is_shell:
        scan = any(r and not _env_file(r) for r in rels)
