...
def check_agent(vault, name):
    ...
    has_host = (root / "CLAUDE.md").is_file() or (root / SHIM_REL).is_dir()
    ...
    if workup_skills is None:
        res.warn(15, "skill-oblig", "no hire workup with a `skills:` field under 03 WiP/, so the "
                 "skill obligation cannot be answered either way")
    ...
    for link in wikilinks(contract):
        ...
        if "hire" in t.lower() and ("research" in t.lower() or "03 WiP" in t):
            brief_links.append(t)
    for m in re.finditer(r"`?(03 WiP/[^`\s\)\]]+)`?", contract):
        t = m.group(1)
        if "hire" in t.lower():
            brief_links.append(t)
...
def find_workup(vault, name):
    ...
    no workup carrying a `skills:` field."""
    wip = vault.root / WIP_REL
    if not wip.is_dir():
        return None, None
    cands = [p for p in sorted(wip.iterdir())
             if p.is_dir() and re.search(r"%s[- ]?hire" % re.escape(name.lower()), p.name.lower())]
    if not cands:
        return None, None
    folder = cands[-1]
...
You are Testy. Research brief: [[03 WiP/2026-09-14-testy-hire/research]].
...
def build_fixture(dest, public=False, scripts_dir=None):
    ...
    (dest / WIP_REL / "2026-09-14-testy-hire").mkdir(parents=True, exist_ok=True)
    (dest / "CLAUDE.md").write_text("@AGENTS.md\n")
    (dest / "AGENTS.md").write_text("# root\n")
    a = dest / AGENTS_REL / "Testy"
    (a / "AGENT.md").write_text(CLEAN_CONTRACT)
    ...
    wip = dest / WIP_REL / "2026-09-14-testy-hire"
...
def _plant_15(v):
    p = v / WIP_REL / "2026-09-14-testy-hire" / "proposal.md"
...
def _strip_both_waivers(v):
    ...
    """
    (v / WIP_REL / "2026-09-14-testy-hire" / "research.md").unlink()
    p = v / WIP_REL / "2026-09-14-testy-hire" / "proposal.md"
...
def self_test(scripts_dir=None, verbose=True):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for mode in ("private", "public"):
            ...
            for num, label, defect, plant, scope in PLANTS:
                if scope not in ("both", mode):
                    continue
                v = tmp / ("%s-%02d-%s" % (mode, num, label))
