...
| `session-start.sh` | NOT the hook entry any more (2026-09-16), and not rendered into any host config. A POSIX shell wrapper: a shell-form hook runs through Git Bash on Windows where it exists and PowerShell where it does not, so it was the reason the ritual never ran there. Kept on disk, still works by hand. Its one non-shell job, the plain line when python3 is missing, moved to `scaffold-init.py doctor` | (nothing renders it) |
