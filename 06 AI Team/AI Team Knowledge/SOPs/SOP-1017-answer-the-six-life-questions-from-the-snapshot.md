...
skill_prerun: 'python3 "06 AI Team/AI Team Knowledge/Scripts/life-snapshot.py" --write --brief'
---
...
is answered by opening `04 Inner World/`.
...
## 1. [SCRIPT] The prerun already ran

The generated skill injects and runs this before step 2:

```
python3 "06 AI Team/AI Team Knowledge/Scripts/life-snapshot.py" --write --brief
```

It writes `.icor-for-life/scripts/snapshot.json` (the machine layer,
...
Both are named on the `session-start-ritual` row in `Scripts/hooks-rules.json`
and both were watched go red before they were trusted:

- `life-snapshot-fixtures`: the fixture suite in `Scripts/test-life-snapshot.py`
