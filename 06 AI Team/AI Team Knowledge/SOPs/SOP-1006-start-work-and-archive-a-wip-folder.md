...
1. On "let's work on X", two decisions, in this order.
   [JUDGEMENT] **The bucket.** Read the list in `03 WiP/README.md` from
...
4. On "done / ship it":
   ...
   `03 WiP/_archive/<the same bucket>/`. The archive mirrors the buckets,
   so the path is kept and nothing is renamed.
5. Work in `03 WiP/` untouched for 30 days is raised in the weekly review
