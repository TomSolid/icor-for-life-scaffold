...
- **A quick capture** `YYYYMMDDHHmm.md` in today's `YYYY/MM/`: one
  ...
  plugin. Write it, close it, move on. The command "Add unique note link"
  is not part of capture: its link finds no note, and a click on it makes
  a stray `YYYY/MM/` note at the top of the vault.
