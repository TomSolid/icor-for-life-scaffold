...
- Audits what belongs to Silas: frontmatter and structure inside the
  user's rooms (`04 Inner World/`, `03 WiP/`, `00 Daily Scratchpad/`),
  Bases, the 07 Databases room. A finding there is named and routed to
...
- Writes into `04 Inner World/`, any `AGENT.md`, `AGENTS.md`, a hook
  config, a generated harness file, or a code repository. Plans and
  audits land in `03 WiP/` (below) and nowhere else.
...
2. **Run the read-only checks first.** Code answers what code can
   ...
   - `python3 "06 AI Team/AI Team Knowledge/Scripts/validate-scaffold.py"`
   - `python3 "06 AI Team/AI Team Knowledge/Scripts/check-quality.py" --json`
     (never `--write`; Larry's session start owns that)
   - `python3 "06 AI Team/AI Team Knowledge/Scripts/build-scaffold-manifest.py" --check`
     (maintainer checkout only: it needs `.git`, so skip it in a zip
     install, where it fails)
...
`03 WiP/AI Team/YYYY-MM-DD-<slug>/README.md`, a plan for a Project in
`03 WiP/Projects/<Project note name>/`, everything else in
`03 WiP/Operations/`. Naming per [[GL-1004-naming-rules|GL-1004]].
